...
from model.peak_stimulation import peak_stimulation
from model.peak_backprop import pr_conv2d
...
class PeakResponseMapping(nn.Sequential):
    ...
    def __init__(self, *args, **kargs):
        ...
        self.enable_peak_stimulation = kargs.get(
        ...
        self.sub_pixel_locating_factor = kargs.get(
    ...
    def _median_filter(input):
        ...
        threshold, _ = torch.median(input.view(
    ...
    def forward(self, input, class_threshold=0, peak_threshold=100, retrieval_cfg=None): ##30
        assert input.dim() == 4, 'PeakResponseMapping layer only supports batch mode.'
        if self.inferencing:
            input.requires_grad_()

        # classification network forwarding
        class_response_maps = super(PeakResponseMapping, self).forward(
            input)  # [N, 20, 14, 14]
        if self.enable_peak_stimulation:
            # sub-pixel peak finding
            if self.sub_pixel_locating_factor > 1:
                class_response_maps = F.upsample(
                    class_response_maps, scale_factor=self.sub_pixel_locating_factor, mode='bilinear', align_corners=True)  # [N, 20, 112, 112]
            # aggregate responses from informative receptive fields estimated via class peak responses
            peak_list, aggregation = peak_stimulation(
                class_response_maps, win_size=self.win_size, peak_filter=self.peak_filter)
        else:
...
def peak_response_mapping(
        ...
        enable_peak_stimulation: bool = True,  # True
        enable_peak_backprop: bool = True,  # True
        win_size: int = 3,
        sub_pixel_locating_factor: int = 1,  # 8
        filter_type: str = 'median') -> nn.Module:
